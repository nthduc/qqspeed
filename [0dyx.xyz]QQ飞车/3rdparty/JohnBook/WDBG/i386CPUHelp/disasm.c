...
void DIdoModrm(PDEBUGPACKET dp,char **, int, BOOL);
...
void GetNextOffset(PDEBUGPACKET, PULONG, BOOL);
...
disasm( PDEBUGPACKET dp, PULONG pOffset, PUCHAR pchDst, BOOL fEAout )
{
...
void
DIdoModrm (PDEBUGPACKET dp, char **ppchBuf, int segOvr, BOOL fEAout)
{
    int     mrm;                        /* modrm byte */
...
OutputSymbol (PDEBUGPACKET dp, char **ppBuf, char *pValue, int length, int segOvr)
{
    ...
    CHAR symBuffer[sizeof(IMAGEHLP_SYMBOL)+1024] ;
...
void
GetNextOffset (PDEBUGPACKET dp, PULONG pcaddr, BOOL fStep)
{
    int     mode_32;
    int     opsize_32;
    int     cBytes;
