...
typedef F32 FloatArrayZ[32][18];
FloatArrayZ *lrSPR = (FloatArrayZ *)SPR_LR_START;
...
//         assert((!(N[cur].child[0] || N[cur].child[1])));
   ...
   for (i=0; i < ( 8192 + 16 ); i++)
      {
      power_fn[i] = powf(F32(i), 4.0F / 3.0F);
      }
...
static S32 L3_requantize(ASISTREAM FAR *STR, S32 gr, S32 ch, is_type FAR * isptr, xr_type FAR* xrptr)
{
   ...
   F32 common = powf(2.0f, (0.25f * (STR->global_gain[gr][ch] - 210.0f)));
...
//   U8  FAR *is_sign = &STR->is_sign[ch][0];
   ...
   for (S32 rgn=0; rgn < 3; rgn++)
      ...
      if (((char*)(is+(end-line)))>(((char*)isptr)+(4*578)))
        end=line+((((char*)isptr)+(4*578))-((char*)is))/sizeof(is[0]);
                               ...
                               F32 k[2][576])
{
    ...
    default:
        if ((is_pos % 2) == 1)
        {
            *k0 = powf(io,(F32) ((is_pos + 1) / 2));
            *k1 = 1;
        }
        else
        {
            *k0 = 1;
            *k1 = powf(io,(F32) (is_pos / 2));
        }
...
static S32 L3_stereo(ASISTREAM FAR *STR, S32 gr, xr_type FAR * xrptr, xr_type FAR * lrptr )
{
...
#ifdef  IS_PS2
   F32 FAR *ld = lrSPR[0][0];
   F32 FAR *rd = lrSPR[1][0];
#else
...
static S32 L3_antialias(ASISTREAM FAR *STR, S32 gr, S32 ch, xr_type FAR * lrptr)
{
   if ((STR->window_switching_flag[gr][ch]) &&
       (STR->block_type           [gr][ch] == 2))
      {
...
{
#ifdef IS_PS2
typedef F32 FloatArray4[2][17][16];
    FloatArray4 *uSPR;
    uSPR = (FloatArray4 *)SPR_U_START;
typedef F32 FloatArrayX[18];
    FloatArrayX *resSPR;
    resSPR = (FloatArrayX *)SPR_RES_START;
#endif
   ...
   for (S32 gr=0; gr < STR->ngr; gr++)
      ...
      //
...
#ifdef IS_PS2
        //
        // DMA STR->lr into scratchpad
        //

        //
        // First make sure the last copy of u from the scratchpad, if any, has completed
        // TODO is this needed/sufficient?
        //
        ...
        //
        // Make sure there's no DMA in progress
        // TODO is this needed/sufficient?
        //
      ...
      S32 ch;
      {
        ...
        for ( ch = 0 ; ch < STR->nch ; ch++ )
        {
           ...
           L3_parse     ( STR, gr, ch, is );

           L3_requantize( STR, gr, ch, is, xr );
        }

#ifdef IS_PS2
        //
        // Make sure the copy of STR->lr to the scratchpad has completed,
        // so it can be used in L3_stereo
        //
      ...
      for (ch=0; ch < STR->nch; ch++)
         ...
         if (win_type == 2)
            ...
            if (use_AMD)
               ...
               for (sb=0; sb < 2; sb++)
                  {
                  AMD_IMDCT_3x12(lr[ch][sb], sb, res[sb], STR->s[ch][sb]);
                  }
               }
            else
#endif
               ...
               for (sb=0; sb < 2; sb++)
                  {
                  x86_IMDCT_3x12(lr[ch][sb], sb, res[sb], STR->s[ch][sb]);
                  }
               }
#else
#ifdef IS_PS2
           for (sb=0; sb < 2; sb++)
               {
               C_IMDCT_3x12(lrSPR[ch][sb], sb, resSPR[sb], STR->s[ch][sb]);
               }
#else
           for (sb=0; sb < 2; sb++)
               {
               PPC_IMDCT_3x12(lr[ch][sb], sb, res[sb], STR->s[ch][sb]);
               }
#endif
#endif
         }
            ...
            if (use_AMD)
               ...
               for (sb=0; sb < 2; sb++)
                  {
                  AMD_IMDCT_1x36(lr[ch][sb], sb, res[sb], STR->s[ch][sb], win[win_type]);
                  }
               }
            else
#endif
               ...
               for (sb=0; sb < 2; sb++)
                  {
                  x86_IMDCT_1x36(lr[ch][sb], sb, res[sb], STR->s[ch][sb], win[win_type]);
                  }
               }
#else
#ifdef IS_PS2
           for (sb=0; sb < 2; sb++)
              {
                 C_IMDCT_1x36(lrSPR[ch][sb], sb, resSPR[sb], STR->s[ch][sb], win[win_type]);
              }
#else
           for (sb=0; sb < 2; sb++)
              {
                 PPC_IMDCT_1x36(lr[ch][sb], sb, res[sb], STR->s[ch][sb], win[win_type]);
              }
#endif
#endif
         }
...
#ifdef IS_PS2
        //
        // Make sure STR->u has finished copying to scratchpad before using it in poly_filter
        //
        ...
        //
        // Make sure there's no copy in progress
        ...
        //
        // copy STR->lr back from scratchpad
        //

        *D8_MADR = (int)STR->lr;
        *D8_SADR = (int)lrSPR;
        *D8_QWC = (sizeof(STR->lr) + 15) >> 4;
        *D8_CHCR = (1<<8);
...
#else
#ifdef IS_PS2
               C_poly_filter(&resSPR[0][0],
                             b,
                             f,
                             &uSPR[ch][div][0][start],
                             &uSPR[ch][!div][0][start]);
#else
               PPC_poly_filter(&res[0][0],
                               b,
                               f,
                               &STR->u[ch][div][0][start],
                               &STR->u[ch][!div][0][start]);
...
#endif
               {
               SSE_dewindow_and_write(STR->u[ch][div][0],
                                      t_dewindow[0],
                                      start,
                                      STR->samples,
                                      STR->nch * sizeof(S16),
...
#else
#ifdef IS_PS2
               PS2_dewindow_and_write(uSPR[ch][div][0],
                                    t_dewindow[0],
                                    start,
                                    STR->samples,
                                    STR->nch * sizeof(S16),
...
#ifdef IS_PS2
        //
        // copy STR->u back from scratchpad
        //
        // When there's only one channel we could probably copy only half of u, but since
        // it's overlapped and a small amount of data, it's pretty much free and simpler
        // to just copy the whole thing
        ...
        //
        // First make sure the previous copy of STR->lr from the scratchpad has completed
        // TODO is this necessary/sufficient?
        //
