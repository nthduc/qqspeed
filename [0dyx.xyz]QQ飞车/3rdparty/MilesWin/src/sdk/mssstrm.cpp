...
DWORD WINAPI stream_thread(LPVOID user)
{
  ...
  while (WaitForSingleObject(threadwait,AIL_preference[DIG_DS_FRAGMENT_SIZE])==WAIT_TIMEOUT)
  {
    MSSLockedIncrement(noreenter);
    if (noreenter==1)
    {
...
HSTREAM  AILCALL AIL_API_open_stream(HDIGDRIVER dig, char const * filename, S32 stream_mem)
{
   ...
   if (streams==0) {
     ...
     if (threadhandle==0)
     {
       ...
       threadhandle=CreateThread(0,65536,stream_thread,0,0,(LPDWORD)&mini);
...
static S32 check_buffers( HSTREAM stream, S32 fillup )
{
  ...
  if ( AIL_sample_buffer_info(stream->samp,&pos0,&len0,&pos1,&len1) )
  {
  ...
  if (stream) {
    ...
    if (stream->preload) {
      ...
      if (stream->loadedsome)
         ...
         AIL_sample_volume_pan(stream->samp,&vol,&pan);
         ...
         AIL_set_sample_type(stream->samp,stream->filetype,stream->fileflags);
         AIL_set_sample_adpcm_block_size(stream->samp,stream->blocksize);
         AIL_set_sample_volume_pan(stream->samp,vol,pan);
...
void  AILCALL AIL_API_set_stream_volume_pan(HSTREAM stream,F32 volume, F32 pan)
{
  if (stream)
    AIL_set_sample_volume_pan(stream->samp,volume,pan);
}
...
void  AILCALL AIL_API_set_stream_volume_levels(HSTREAM stream,F32 left, F32 right)
{
  if (stream)
    AIL_set_sample_volume_levels(stream->samp,left,right);
}


//############################################################################
//##                                                                        ##
//## Change the reverb of a streaming file                                  ##
//##                                                                        ##
//############################################################################

void  AILCALL AIL_API_set_stream_reverb_levels(HSTREAM stream,F32 dry, F32 wet)
{
  if (stream)
    AIL_set_sample_reverb_levels(stream->samp,dry,wet);
}


//############################################################################
//##                                                                        ##
//## Change the cutoff of a streaming file                                  ##
//##                                                                        ##
//############################################################################

void  AILCALL AIL_API_set_stream_low_pass_cut_off(HSTREAM stream,F32 cutoff)
{
  if (stream)
    AIL_set_sample_low_pass_cut_off(stream->samp,cutoff);
}
...
void  AILCALL AIL_API_stream_volume_pan(HSTREAM stream,F32 FAR * volume, F32 FAR * pan)
{
  if ( stream )
  {
    AIL_sample_volume_pan( stream->samp, volume, pan );
  }
  else
  {
    if ( volume )
...
//############################################################################
//##                                                                        ##
//## Get the volume of a streaming file                                     ##
//##                                                                        ##
//############################################################################

void  AILCALL AIL_API_stream_volume_levels(HSTREAM stream,F32 FAR * left, F32 FAR * right)
{
  if ( stream )
  {
    AIL_sample_volume_levels( stream->samp, left, right );
  }
  else
  {
    if ( left )
...
//############################################################################
//##                                                                        ##
//## Get the reverb of a streaming file                                     ##
//##                                                                        ##
//############################################################################

void  AILCALL AIL_API_stream_reverb_levels(HSTREAM stream,F32 FAR * dry, F32 FAR * wet)
{
  if ( stream )
  {
    AIL_sample_reverb_levels( stream->samp, dry, wet );
  }
  else
  {
    if ( dry )
...
//############################################################################
//##                                                                        ##
//## Get the cutoff of a streaming file                                     ##
//##                                                                        ##
//############################################################################

F32  AILCALL AIL_API_stream_low_pass_cut_off(HSTREAM stream)
{
  if ( stream )
  {
    return( AIL_sample_low_pass_cut_off( stream->samp ) );
  }
  else
  {
    return( 1.0f );
