...
static void DLS_VOICE_note_on(VOICE FAR *V, MIDI_MESSAGE FAR *EV, S32 key, REGION FAR *RGN)
{
   ...
   if (EG1_scale == ABSOLUTE_ZERO)
      ...
      V->EG1_lin_scale = (F32) pow(10.0F, V->EG1_scale / 20.0F);
...
static void AILLIBCALLBACK DLS_serve(U32 user)
{
   ...
   for (S32 vn=0; vn < DLS->n_voices; vn++)
         ...
         switch (V->EG1_phase)
            ...
            case ATTACK_PHASE:
               ...
               if (V->EG1_attack_intervals > 0)
                  ...
                  if (result > 0.0F)
                     {
                     V->EG1_atten = -20.0F * (F32) log10(result / 65536.0F);
                     }
      ...
      if (V->mixer_operation & M_DEST_STEREO)
         ...
         if (left_atten > 0.0F)
            {
            left_scale = ((S32)(2048.0F / pow(10.0F, left_atten  / 20.0F)));
            }
         else
            {
            left_scale = 2048;
         ...
         if (right_atten > 0.0F)
            {
            right_scale = ((S32)(2048.0F / pow(10.0F, right_atten / 20.0F)));
            }
         else
            {
            right_scale = 2048;
...
*/
         ...
         if (mono_atten > 0.0F)
            {
            left_scale = ((S32)(2048.0F / pow(10.0F, mono_atten / 20.0F)));
            }
         else
            {
            left_scale = 2048;
        ...
        else
        {
          ...
          if ( needed_src >= have_src )
          {
            ...
            S32 dest_from_src = shift64addsubanddiv( have_src >> src_sample_shift, 16,
                                                     playback_ratio - 1, V->src_fract & 0xffff,
