...
//##  Miles Sound System                                                    ##
//##                                                                        ##
//##  API.CPP: FLT module for Dolby Surround                                ##
//##                                                                        ##
...
static U32 AILEXPORT PROVIDER_query_attribute (HATTRIB index)
{
   switch ((ATTRIB) index)
      {
      case PROVIDER_NAME:      return (U32) "Dolby Surround Filter";
                                           ...
                                           HSAMPLE      S)
{
   ...
   // init default values
   SS->delay =  MAX_DELAY;
   SS->buffer_pos = SS->buffer;
   SS->buffer_end = SS->buffer + SS->delay;
   SS->delay  =  MAX_DELAY;
   SS->mix    =  0;
   SS->delayf =  MAX_DELAY;
   SS->mixf   =  0;
...
static S32 __inline mult64andshifts( S32 mt1, S32 mt2, S32 shift )
{
  S32 retv;
...
#define mul64shilo(_hi,_lo,_mt1,_mt2)    \
  __asm                                  \
  {                                      \
    mulhw _hi, _mt1, _mt2;               \
    mullw _lo, _mt1, _mt2                \
...
static U32 inline mult64andshifts( S32 m1,S32 m2,S32 shift )
{
  register S32 hi;
  register U32 lo;
  register S32 mt1, mt2;

  mt1=m1;
  mt2=m2;

  mul64shilo( hi, lo, mt1, mt2 );
                                    ...
                                    U32             mixer_operation)
{
   ...
   if ( ( ( mixer_operation & M_DEST_STEREO ) == 0 ) || ( SS->mix == 0 ) )
   {
     // if mono output or all in the front, let the mixer do everything
     return( 1 );
   }
   ...
   for( dwIndex = 0; dwIndex < dwTemp; dwIndex += 2 )
   {
      ...
      // get input samples
      il = dwTempBuild[ dwIndex ];
      ir = dwTempBuild[ dwIndex + 1 ];

      b = *SS->buffer_pos;
      *SS->buffer_pos++ = ( il + ir ) / 2;

      if ( SS->buffer_pos >= SS->buffer_end )
        SS->buffer_pos = SS->buffer;

      // store output
      lpBuildDest[ dwIndex ]      += ( il + mult64andshifts( mix, b - il, 16 ) );
      lpBuildDest[ dwIndex + 1 ]  += ( ir - mult64andshifts( mix, b + ir, 16 ) );
   }
   ...
   switch ((ATTRIB) attribute)
      {
         case  _FX_DOLBY_DELAY:
               return( (S32) float_as_long( SS->delayf ));

         case  _FX_DOLBY_MIX:
               return( (S32) float_as_long( SS->mixf ));
      }
                                            ...
                                            void FAR*    value)
{
   ...
   switch ((ATTRIB) preference)
      ...
      case  _FX_DOLBY_DELAY:
            ...
            prev        = (S32) float_as_long( SS->delayf );
            SS->delayf  = *(F32 FAR*)value;
            
            if ( SS->delayf <= 0.0001f )
              SS->delayf = 0.0f;
            else if ( SS->delayf > MAX_DELAY )
              SS->delayf = MAX_DELAY;

            SS->delay   = (S32) SS->delayf;
      ...
      case  _FX_DOLBY_MIX:
            // get previous value and set new
            prev        = (S32) float_as_long( SS->mixf );
            SS->mixf    = *(F32 FAR*)value;

            if ( SS->mixf <= 0.0001f )
              SS->mixf = 0.0f;
            else if ( SS->mixf > 0.9999f )
              SS->mixf = 1.0f;

            SS->mix     = (S32) ( SS->mixf * 65536.0f );
            break;
...
extern "C" S32 DolbyMain( HPROVIDER provider_handle, U32 up_down )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP[] =
      ...
      REG_AT("Rear Delay",      _FX_DOLBY_DELAY,         RIB_FLOAT),
      REG_PR("Rear Delay",      _FX_DOLBY_DELAY,         RIB_FLOAT),
      REG_AT("FB Pan",          _FX_DOLBY_MIX,           RIB_FLOAT),
      REG_PR("FB Pan",          _FX_DOLBY_MIX,           RIB_FLOAT),
      };
