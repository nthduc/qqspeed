...
#ifndef IS_DOS
DXDEF
S32 AILCALL AIL_find_filter( char const * name, HPROVIDER * ret )
{
...
DXDEF
S32 AILCALL AIL_find_3D_provider( char const * name, HPROVIDER * ret )
{
  HPROENUM enum3Ds = HPROENUM_FIRST;
  HPROVIDER cur_prov;
  char * m3d_name;
  
  while (AIL_API_enumerate_3D_providers( &enum3Ds,
                                         &cur_prov,
                                         &m3d_name ) )
  {
    if (AIL_strcmp(m3d_name,name)==0)  
    {
      if (ret)
        *ret = cur_prov;
      return( 1 );
