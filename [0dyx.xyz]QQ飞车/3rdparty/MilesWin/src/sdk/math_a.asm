...
x86_dewindow_and_write PROC \
        ...
        mov ebx,[lpDewindow]
        lea ebx,[ebx + eax*4 + 16*4]    ; adjust up by 16-dwStart input values
        ...
        lea edx,[esi][16*16*4]          ; end address - we'll do 16 samples, each using a
...
__for_loops:
    ...
    IFDEF SSE   ;------------------------------------------------
        ...
        MAC_INIT_PAIR_SSE   [esi],[ebx]
        CNT = 16

        REPT 3

            MAC_PAIR_SSE  [esi+CNT],[ebx+CNT]
            CNT = CNT + 16
    ...
    ELSE    ; non-SSE -------------------------------------------
        ...
        MAC_INIT_PAIR  [esi],[ebx],[esi+4],[ebx+4]
        CNT = 8
...
__odd_loops:
        ...
        ASCEND = 0*4
        DESCEND = 15*4
        MAC_INIT_PAIR_SWAPPED_4_SSE [esi+ASCEND-16*4],[ebx+DESCEND-12]

        REPT 3

            DESCEND = DESCEND - 16
            ASCEND = ASCEND + 16
            MAC_PAIR_SWAPPED_4_SSE    [esi+ASCEND-16*4],[ebx+DESCEND-12]
        ...
        ASCEND = 0*4
        DESCEND = 15*4
        MAC_INIT_PAIR_SWAPPED_2_SSE [esi+ASCEND-16*4],[ebx+DESCEND-12]

        REPT 3

            DESCEND = DESCEND - 16
            ASCEND = ASCEND + 16
            MAC_PAIR_SWAPPED_2_SSE    [esi+ASCEND-16*4],[ebx+DESCEND-12]
        ...
        ASCEND = 0*4
        DESCEND = 15*4
        MAC_INIT_PAIR_SWAPPED_1_SSE   [esi+ASCEND-16*4-16*4*2],[ebx+DESCEND-12-32*4*2]

        REPT 3

            DESCEND = DESCEND - 16
            ASCEND = ASCEND + 16
            MAC_PAIR_SWAPPED_1_SSE  [esi+ASCEND-16*4-16*4*2],[ebx+DESCEND-12-32*4*2]
...
__odd_loops:
        ...
        MAC_INIT_PAIR [esi+ASCEND+4-16*4],[ebx+DESCEND-4],[esi+ASCEND-16*4],[ebx+DESCEND]
        ...
        REPT 7
            ...
            MAC_PAIR [esi+ASCEND+4-16*4],[ebx+DESCEND-4],[esi+ASCEND-16*4],[ebx+DESCEND]
...
__even_loops:
        ...
        ASCEND = 0*4
        DESCEND = 15*4
        MAC_INIT_PAIR [esi+ASCEND-16*4],[ebx+DESCEND],[esi+ASCEND+4-16*4],[ebx+DESCEND-4]

        REPT 7

            DESCEND = DESCEND - 8
            ASCEND = ASCEND + 8
            MAC_PAIR [esi+ASCEND-16*4],[ebx+DESCEND],[esi+ASCEND+4-16*4],[ebx+DESCEND-4]
...
db 'Copyright (C) 1991-2004 RAD Game Tools, Inc.',13,10,13,10
