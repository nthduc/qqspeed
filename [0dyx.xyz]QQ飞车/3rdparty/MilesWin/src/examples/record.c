...
static void AILCALLBACK handle_sound( void const * data, S32 bytes, U32 user_data )
{
  S32 left = total_size - current_size;
  ...
  memcpy( ( (U8*) input_buf ) + current_size, data, bytes );
  current_size += bytes;
}
