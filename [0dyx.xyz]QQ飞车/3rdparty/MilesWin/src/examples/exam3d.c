...
static void set_speaker( S32 which )
{
  if ( opened_provider )
  {
    ...
    // note that if nothing is checked, then we don't set the speaker at all. 
    // this allows the system setting to be inherited if nothing is selected.
    ...
    if ( which == 1 )
      ...
      if ( which == 2 )
        ...
        if ( which == 3 )
          ...
          if ( which == 4 )
            speaker_type = AIL_3D_51_SPEAKER;

     if (  speaker_type != -1 )
        AIL_set_3D_speaker_type( opened_provider, speaker_type );
   
     // now read back what was set...
     speaker_type = AIL_3D_speaker_type( opened_provider );
   
     Check_control( rad2Speaker,  ( speaker_type == AIL_3D_2_SPEAKER ) ? 1 : 0 );
     Check_control( radHeadphone, ( speaker_type == AIL_3D_HEADPHONE ) ? 1 : 0 );
     Check_control( rad4Speaker,  ( speaker_type == AIL_3D_4_SPEAKER ) ? 1 : 0 );
     Check_control( rad51Speaker, ( speaker_type == AIL_3D_51_SPEAKER ) ? 1 : 0 );
  }
}
...
static void set_provider_controls()
{
  ...
  Enable_control(rad51Speaker,open);
...
static void move_sample_values( U32 time )
{
  F32 i;

  i = ( (F32) get_slider_value() ) / 128.0f;
  i = ( i * ( FASTSPEED - SLOWSPEED ) ) + SLOWSPEED;
  i = i * (F32) time;
  ...
  else 
  {
    ...
    //handle the back and forth movement
    if ( Z > STRAIGHTDIST )
      adj = -1.0F;
...
static void set_sample_values()
{
  ...
  AIL_set_3D_velocity(opened_sample, dX, dY, dZ, i);
...
static void set_new_provider(S32 index)
{
  ...
  else
  {
    ...
    else
    {
      ...
      AIL_set_3D_sample_distances(opened_sample,200,40);
      AIL_set_3D_sample_cone(opened_sample,90,120,0.0f);

      AIL_3D_sample_cone(opened_sample,0,0,&fresult);
      nocone=(fresult!=0.0f)?1:0;
      ...
      case WM_COMMAND:
         ...
         switch (LOWORD(wParam))
         {
          ...
          case btnPlay:
             if (opened_sample)
             {
               firsttime = lasttime = AIL_ms_count();
