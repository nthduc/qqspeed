...
static void Update_3D_volume( H3DSAMPLE sample, F32 dv )
{
  if ( sample == NULL )
  {
    return;
  ...
  F32 i = AIL_3D_sample_volume( sample );
      ...
      if (kbhit())
      {
         ...
         switch (ch)
         {
            ...
            case 'm': case 'M':
               ...
               for (i=0; i < n_3D; i++)
                  {
                  AIL_set_3D_sample_loop_count( h3D[i], loop_count );
                  }

               for (i=0; i < n_2D; i++)
                  {
                  AIL_set_sample_loop_count( h2D[i], loop_count );
                  }
            ...
            case 'i': case 'I':
              Update_3D_position( last_3D, 0.0F, 0.0F, 1.0F );
              break;
            ...
            case 'j': case 'J':
              Update_3D_position( last_3D, -1.0F, 0.0F, 0.0F );
              break;
            ...
            case 'y': case 'Y':
              Update_3D_position( last_3D, 0.0F, 1.0F, 0.0F );
              break;
            ...
            case '-': case '_':
               Update_3D_volume( last_3D, 1.0f/1.05f );
               break;

            case '=': case '+':
               Update_3D_volume( last_3D, 1.05f );
               break;
            ...
            case 's': case 'S':
               ...
               if (i == n_3D)
               {
                  printf("No H3DSAMPLEs available\n");
                  break;
               }

               if (!AIL_set_3D_sample_file(h3D[i],
                                           w2))
                  {
                  //
                  // Always check the return value from AIL_set_3D_sample_file() and
                  // AIL_set_3D_sample_info() -- they can fail if a hardware provider
                  // runs out of system-wide voices!
                  //
