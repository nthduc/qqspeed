...
protected:
	...
	INIT_INTERFACE_PART(CPingMgr, PingMgr)
		STDMETHOD(SetLimitTime)(int nLimitTime);
	    STDMETHOD(PingServer)(unsigned long ulSvrIP, unsigned short usPort, int *pnIndex);
	...
	STDMETHOD(SetLimitTime)(int nLimitTime);
	STDMETHOD(PingServer)(unsigned long ulSvrIP, unsigned short usPort, int *nIndex);
	STDMETHOD(CancelPing)(int nIndex);
	STDMETHOD(Initialize)(IPingSink *pPingSink);	
